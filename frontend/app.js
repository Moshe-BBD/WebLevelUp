document.addEventListener('DOMContentLoaded', () => {
    ...
    stubSpiders.forEach((spider, index) => {
        ...
        const pageNumber = document.createElement('span'); // Create a span for the page number
        ...
        likeBtn.innerHTML = '❤';
        pageNumber.textContent = `Page ${index + 1} of ${stubSpiders.length}`; // Setting the page number
        ...
        card.addEventListener('click', () => {
            document.querySelectorAll('#carousel article').forEach(c => c.classList.remove('active'));
    ...
    function centerCard(selectedCard) {
        const activeCardOffset = selectedCard.offsetLeft + selectedCard.offsetWidth / 2;
